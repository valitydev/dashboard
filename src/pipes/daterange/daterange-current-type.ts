...
export const isCurrentYear = ({ begin, end }: Daterange) =>
    isYear({ begin, end }) && moment().isSame(begin, 'year');

export const isCurrentMonth = ({ begin, end }: Daterange) =>
    isMonth({ begin, end }) && begin.isSame(moment(), 'month');

export const isCurrentWeek = ({ begin, end }: Daterange) =>
    begin.isSame(moment().startOf('week'), 'day') && end.isSame(moment().endOf('week'), 'day');

export const isToday = ({ begin, end }: Daterange) =>
    isDay({ begin, end }) && begin.isSame(moment(), 'day');
